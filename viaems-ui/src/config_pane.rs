use crate::interface;
use crate::table_editor::{Table1dEditor, Table2dEditor};

fn render_single_optional_value_input<T: emath::Numeric + Default>(
    ui: &mut egui::Ui,
    reference: &Option<T>,
    field: &mut Option<T>,
    name: &str,
...
fn render_single_value_input<T: emath::Numeric>(
    ui: &mut egui::Ui,
    reference: &T,
    field: &mut T,
    name: &str,
    ...
    ui.horizontal(|ui| {
        ui.label(name);
        ui.add(egui::DragValue::new(field).update_while_editing(false));
        if reference != field {
            if ui
...
fn render_enum_selector<T: PartialEq + Copy>(
    ui: &mut egui::Ui,
...
fn render_sensor(
    ui: &mut egui::Ui,
    name: &str,
    live_sensor: &Option<interface::configuration::Sensor>,
    sensor: &mut Option<interface::configuration::Sensor>,
) {
...
fn render_knock_sensor(
    ui: &mut egui::Ui,
    name: &str,
    live_sensor: &Option<interface::configuration::KnockSensor>,
    sensor: &mut Option<interface::configuration::KnockSensor>,
) {
    let sensor = sensor.get_or_insert_default();
    let sensor_ref = live_sensor.clone().unwrap_or_default();
    ...
    egui::CollapsingHeader::new(red_if_changed(&sensor_ref, sensor, name)).show(ui, |ui| {
        ...
        render_single_optional_value_input(
            ui,
            &sensor_ref.frequency,
            &mut sensor.frequency,
            "Frequency",
        );
        render_single_optional_value_input(
            ui,
            &sensor_ref.threshold,
            &mut sensor.threshold,
            "Threshold",
        );
    });
}

pub fn render_config_pane(
    ui: &mut egui::Ui,
    live_config: &interface::Configuration,
    local_config: &mut interface::Configuration,
) {
    egui::ScrollArea::vertical().show(ui, |ui| {
        egui::CollapsingHeader::new(red_if_changed(
            &live_config.outputs,
            &local_config.outputs,
            "Outputs",
        ))
        .show(ui, |ui| {
            egui::Grid::new("outputlist")
                ...
                .show(ui, |ui| {
                    ...
                    for (idx, output) in local_config.outputs.iter_mut().enumerate() {
                        ...
                        ui.add(
                            egui::DragValue::new(output.pin.get_or_insert_default())
                                .update_while_editing(false),
                        );
                        ui.add(
                            egui::DragValue::new(output.angle.get_or_insert_default())
                                .update_while_editing(false),
                        );
        ...
        egui::CollapsingHeader::new(red_if_changed(&sensors_ref, sensors, "Sensors")).show(
            ui,
            |ui| {
        ...
        egui::CollapsingHeader::new(red_if_changed(&ignition_ref, ignition, "Ignition")).show(
            ui,
            |ui| {
                render_enum_selector(
                    ui,
                ...
                match ignition.r#type() {
                    interface::configuration::ignition::DwellType::DwellFixedDuty => {
                        render_single_optional_value_input(
                            ui,
                            &ignition_ref.fixed_duty,
                            &mut ignition.fixed_duty,
                            "Dwell Duty (%)",
                        );
                    }
                    interface::configuration::ignition::DwellType::DwellFixedTime => {
                        render_single_optional_value_input(
                            ui,
                            &ignition_ref.fixed_dwell,
                            &mut ignition.fixed_dwell,
                            "Dwell Time (uS)",
                        );
                    }
                    interface::configuration::ignition::DwellType::DwellBrv => {
                ...
                render_single_optional_value_input(
                    ui,
                    &ignition_ref.ignitions_per_cycle,
                    &mut ignition.ignitions_per_cycle,
                    "Ignitions per cycle",
                );
                render_single_optional_value_input(
                    ui,
                    &ignition_ref.min_coil_cooldown_us,
                    &mut ignition.min_coil_cooldown_us,
                    "Coil cooldown (uS)",
                );
                render_single_optional_value_input(
                    ui,
                    &ignition_ref.min_dwell_us,
                    &mut ignition.min_dwell_us,
                    "Minimum Dwell (uS)",
                );
        ...
        egui::CollapsingHeader::new(red_if_changed(&fueling_ref, fueling, "Fueling")).show(
            ui,
            |ui| {
                render_single_optional_value_input(
                    ui,
                    &fueling_ref.fuel_pump_pin,
                    &mut fueling.fuel_pump_pin,
                    "Fuelpump pin",
                );
                render_single_optional_value_input(
                    ui,
                    &fueling_ref.cylinder_cc,
                    &mut fueling.cylinder_cc,
                    "Cylinder CC",
                );
                render_single_optional_value_input(
                    ui,
                    &fueling_ref.fuel_density,
                    &mut fueling.fuel_density,
                    "Fuel Density",
                );
                render_single_optional_value_input(
                    ui,
                    &fueling_ref.fuel_stoich_ratio,
                    &mut fueling.fuel_stoich_ratio,
                    "Fuel Stoich Ratio",
                );
                render_single_optional_value_input(
                    ui,
                    &fueling_ref.injections_per_cycle,
                    &mut fueling.injections_per_cycle,
                    "Injections per cycle",
                );
                render_single_optional_value_input(
                    ui,
                    &fueling_ref.injector_cc,
                    &mut fueling.injector_cc,
                    "Injector CC/min",
                );
                render_single_optional_value_input(
                    ui,
                    &fueling_ref.max_duty_cycle,
                    &mut fueling.max_duty_cycle,
                    "Max Duty Cycle (%)",
                );

                let ce = fueling.crank_enrich.get_or_insert_default();
                let ce_ref = fueling_ref.crank_enrich.clone().unwrap_or_default();
                egui::CollapsingHeader::new(red_if_changed(&ce_ref, ce, "Cranking Enrichment"))
                    .default_open(true)
                    .show(ui, |ui| {
                        render_single_optional_value_input(
                            ui,
                        ...
                        render_single_optional_value_input(
                            ui,
                            &ce_ref.cranking_temp,
                            &mut ce.cranking_temp,
        ...
        egui::CollapsingHeader::new(red_if_changed(&decoder_ref, decoder, "Decoder")).show(
            ui,
            |ui| {
                render_enum_selector(
                    ui,
                ...
                render_single_optional_value_input(
                    ui,
                    &decoder_ref.degrees_per_trigger,
                    &mut decoder.degrees_per_trigger,
                    "Degrees per trigger",
                );
                render_single_optional_value_input(
                    ui,
                    &decoder_ref.max_tooth_variance,
                    &mut decoder.max_tooth_variance,
                    "Max tooth variance",
                );
                render_single_optional_value_input(
                    ui,
                ...
                render_single_optional_value_input(
                    ui,
                    &decoder_ref.num_triggers,
                    &mut decoder.num_triggers,
                    "Trigger count",
                );
                render_single_optional_value_input(
                    ui,
        ...
        egui::CollapsingHeader::new(red_if_changed(&rpm_cut_ref, &rpm_cut, "Rpm Cut")).show(
            ui,
            |ui| {
                render_single_optional_value_input(
                    ui,
                    &rpm_cut_ref.rpm_limit_start,
                    &mut rpm_cut.rpm_limit_start,
                    "RPM Lower Hysteresis",
                );
                render_single_optional_value_input(
                    ui,
                    &rpm_cut_ref.rpm_limit_stop,
                    &mut rpm_cut.rpm_limit_stop,
                    "RPM Upper Hysteresis",
                );
            },
        );
        ...
        egui::CollapsingHeader::new(red_if_changed(&cel_ref, &cel, "Check Engine Light")).show(
            ui,
            |ui| {
                render_single_optional_value_input(ui, &cel_ref.pin, &mut cel.pin, "Pin");
                render_single_optional_value_input(
                    ui,
                    &cel_ref.lean_boost_ego,
                    &mut cel.lean_boost_ego,
                    "Max EGO-in-boost",
                );
                render_single_optional_value_input(
                    ui,
                    &cel_ref.lean_boost_map_enable,
                    &mut cel.lean_boost_map_enable,
                    "EGO-in-boost MAP threshold (kpa)",
                );
            },
        );
        ...
        egui::CollapsingHeader::new(red_if_changed(&boost_ref, &boost, "Boost Control")).show(
            ui,
            |ui| {
                render_single_optional_value_input(ui, &boost_ref.pin, &mut boost.pin, "Pin");
                render_single_optional_value_input(
                    ui,
                    &boost_ref.control_threshold_map,
                    &mut boost.control_threshold_map,
                    "Lower MAP threshold (kpa)",
                );
                render_single_optional_value_input(
                    ui,
                    &boost_ref.control_threshold_tps,
                    &mut boost.control_threshold_tps,
                    "Lower TPS threshold (%)",
                );
                render_single_optional_value_input(
                    ui,
                    &boost_ref.enable_threshold_map,
                    &mut boost.enable_threshold_map,
                    "Enable MAP threshold (kpa)",
                );
                render_single_optional_value_input(
                    ui,
                    &boost_ref.overboost_map,
                    &mut boost.overboost_map,
                    "Overboost limit (kpa)",
                );
