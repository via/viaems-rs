...
pub fn detect(mcast_dest: SocketAddrV4, timeout: Option<Duration>) -> Vec<UdpDevice> {
    ...
    for laddr in &local_addrs {
        ...
        socket
            .join_multicast_v4(&mcast_dest.ip(), laddr)
            .expect("unable to join mcast group");
        ...
        if let Ok((_, source)) = socket.recv_from(&mut rcvbuf) {
            let result = UdpDevice {
                target_ucast_ipaddr: if let std::net::SocketAddr::V4(v4) = source {
                    v4
                } else {
                    unreachable!()
                },
...
impl Connection {
    pub fn new_udp(device: &UdpDevice) -> Connection {
        let socket = UdpSocket::bind((device.local_ipaddr, device.target_mcast_ipaddr.port()))
            .expect("socket");
        socket
            .join_multicast_v4(&device.target_mcast_ipaddr.ip(), &device.local_ipaddr)
            .unwrap();
...
fn send_loop(
    ...
    _addr: SocketAddrV4,
    rx: mpsc::Receiver<interface::Message>,
) {
    loop {
